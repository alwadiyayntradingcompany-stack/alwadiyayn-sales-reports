 ...
 * 
 * خطوات الإعداد:
 * 1. أنشئ Google Sheet جديد وانسخ الـ ID
 * 2. أنشئ مجلد في Google Drive وانسخ الـ ID  
 * 3. ضع الـ IDs في المتغيرات أدناه
 * 4. احفظ ونشر كـ Web App
 * 5. انسخ URL الجديد وضعه في ALWADIYAYNFORM.js
 */

// ضع معرفات Google Sheet و Drive هنا
const SHEET_ID = 'ضع_معرف_الشيت_هنا'; // مثال: 1D8o5biC3BDK6neb1PCDDOejK5S4kzV6xVT4BbQ3kYAE
const FOLDER_ID = 'ضع_معرف_المجلد_هنا'; // مثال: 1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms
...
function doPost(e) {
  try {
    ...
    Logger.log('📊 البيانات المستلمة: ' + JSON.stringify(data));
    ...
    // إضافة الصف
    sheet.appendRow(row);
    Logger.log('✅ تم حفظ البيانات في الشيت');
...
function doGet() {
  Logger.log('📡 طلب GET - اختبار الاتصال');
  
  const response = {
    status: 'active',
    message: 'نظام تقارير المبيعات يعمل بشكل مثالي! ✅',
    timestamp: new Date().toISOString(),
    version: '2026.1',
...
function getCurrentHijriDate() {
  try {
    ...
    Logger.log('خطأ في التاريخ الهجري: ' + error);
    return '01-01-1446';
  }
}
